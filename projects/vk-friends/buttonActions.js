...
document.addEventListener('click', function (e) {
  ...
  if (e.target.getAttribute('data-role') === 'add') {
    ...
    bestContainer.insertBefore(
      ...
      bestContainer.querySelector('.friend')
    );
    ...
    // Проверка, есть ли уже такой id в localStorage
    let isExist = false;
    ...
    // Если такого id еще нет, записываем в localStorage
    if (!isExist) {
    ...
    if (e.target.getAttribute('data-role') === 'delete') {
      ...
      allContainer.insertBefore(
        e.target.parentElement.parentElement,
        allContainer.querySelector('.friend')
      );
